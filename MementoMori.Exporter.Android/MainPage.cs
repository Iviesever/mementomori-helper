...
public sealed class MainPage : ContentPage
{
    ...
    private readonly Label output = Body("默认生成一个精简 JSON，生成后可保存到手机或分享。");
    ...
    private readonly Button forget = ActionButton("清除本机登录配置和临时文件", secondary: true);
    ...
    public MainPage(MobileSession session)
    {
        ...
        {
            // Do not show a previous account's file or summary under the new selection.
            ClearLastExport();
            UpdateSnapshot();
            UpdateButtons();
        };
        ...
        var optionsCard = Card(Heading("03  生成、保存与分享"),
            ...
            Body("重复生成使用上次读取的账号快照。游戏内变更后，请再次点“读取账号”；文件生成时间不代表数据读取时间。卡池列表单独读取。"));
        var footer = Card(
            Body("独立运行，不需要电脑在线。不提供自动抽卡、领奖或战斗入口。读取时建议先完成游戏内操作，避免并发登录影响会话。"),
            Body("保存到公共目录或分享出去的副本由你管理，清除本机配置不会删除这些副本。"), forget);
        Content = new ScrollView { Content = new VerticalStackLayout
        {
            Spacing = 14, Padding = new Thickness(18, 18, 18, 32),
            Children = { Heading("MementoMori", 28), Body("手机账号导出 · 预览版 0.2.0"), accountCard, sectionsCard, optionsCard, footer }
        } };
    ...
    private async Task ReadAccountAsync()
    {
        ...
        try
        {
            ...
            status.Text = session.FetchedAtUtc == null ? session.Summary : "读取完成，选择需要的内容后生成文件。";
        }
    ...
    private async Task SaveLastAsync()
    {
        ...
        try
        {
            // A cloud/local provider can block when opening its stream; keep this off the UI thread.
        ...
        {
            // CREATE_DOCUMENT creates a new file. Best-effort removal avoids an empty/partial copy.
            try { global::Android.Provider.DocumentsContract.DeleteDocument(resolver, uri); }
            catch (Exception) { /* Some providers cannot remove their incomplete document. */ }
    ...
    {
        if (!await DisplayAlert("清除本机配置", "将清除本 App 保存的登录配置和临时导出文件，不会删除游戏账号，也不会删除已经保存或分享出去的副本。", "清除", "取消")) return;
        ClearLastExport();
        try { await session.ForgetAsync(); }
        finally { RefreshAccounts(); UpdateSnapshot(); }
        status.Text = "本机登录配置和临时文件已清除。可以重新导入。";
    }
    ...
    private async Task RunUiAsync(Func<Task> action, bool restoringCredentials = false)
    {
        ...
        {
            status.Text = "操作已取消、超时或被系统中断，可以重试。";
        }
        catch (Exception e)
        {
            // Never display remote messages, request bodies, paths or credential values.
            var message = restoringCredentials
                ? "本机保存的登录配置无法读取。可重新导入；若仍失败，请先点“清除本机登录配置和临时文件”。"
                : e switch
                {
                    InvalidDataException => "配置或文件格式不正确，请重新选择私有登录配置。",
                    JsonException => "登录配置不是有效 JSON。",
                    FileNotFoundException => "临时文件已失效或文件无法读取，请重新生成或重新选择文件。",
                    HttpRequestException => "网络请求失败，请检查网络后重试。已有配置不会因此删除。",
                    IOException => "文件读写未完成，请检查空间和保存位置后重试。目标目录可能留有不完整副本。",
                    _ => $"操作未完成（{e.GetType().Name}）。可重试，或清除本机配置后重新导入。"
                };
            status.Text = message;
    ...
    private void UpdateButtons()
    {
        ...
        // Also available after a secure-storage read failure, even with zero loaded accounts.
