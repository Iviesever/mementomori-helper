...
try {
    ...
    Copy-Item "$PSScriptRoot/Start-Exporter.ps1","$PSScriptRoot/START-EXPORTER.cmd" -Destination $stage
    Copy-Item "$root/LICENSE" -Destination $stage
    @'
MementoMori Exporter - Windows x64 preview
Extract the whole ZIP, then open START-EXPORTER.cmd. No .NET SDK/runtime install is needed.
...
Offline installation check: powershell -NoProfile -File Start-Exporter.ps1 -OfflineCheck
...
'@ | Set-Content "$stage/README.txt" -Encoding utf8
    ...
    $hashes = @(Get-ChildItem $stage -Recurse -File | Sort-Object FullName | ForEach-Object {
