...
# Parse only verified APK signer certificates, never a source stamp or public-key digest.
# AOSP emits either numbered signers or SDK-ranged signers for APK Signature Scheme v3.1.
...
$pattern = '(?m)^Signer (?:#\d+|\(minSdkVersion=[^\r\n]+\)) certificate SHA-256 digest:\s*([A-Fa-f0-9]{64})\s*\r?$'
