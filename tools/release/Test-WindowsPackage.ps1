...
try {
    ...
    $process = Start-Process powershell.exe -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',"`"$unpacked/Start-Exporter.ps1`"",'-OfflineCheck','-NoBrowser','-Port',[string]$port) -RedirectStandardOutput "$out/windows-offline-stdout.txt" -RedirectStandardError "$out/windows-offline-stderr.txt" -PassThru
    $url="http://127.0.0.1:$port"
    $response=$null
    for ($i=0; $i -lt 90; $i++) {
        $process.Refresh()
        if ($process.HasExited) { throw 'Packaged launcher exited before becoming ready. See isolated offline diagnostics.' }
    ...
    @{ passed=$true; platform='Windows Server 2022 CI'; checks=$checks; realGameLogin=$false } | ConvertTo-Json | Set-Content "$out/windows-smoke.json" -Encoding utf8
} finally {
