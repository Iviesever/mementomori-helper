...
Extract the entire ZIP, then run the outer `START-EXPORTER.cmd` beside the `app` folder. Keep them together; the CMD resolves `app/Start-Exporter.ps1` relative to its own location, not the current working directory. It includes its .NET runtime; no SDK, git checkout or separately installed .NET runtime is needed. This is an unsigned Windows preview, not an Authenticode-signed installer.

Normal use needs your existing private desktop login configuration at `%LOCALAPPDATA%\MementoMoriExporter\appsettings.user.json`. It must contain a valid AuthOption. Portable mode forces GameConfig.AutoJob.DisableAll=true independently of the original file. Alternatively run `START-EXPORTER.cmd -ConfigPath <your-private-path>` from the outer directory. Never send this file to chat, GitHub or a bug report.
...
CI runs the actual outer CMD launcher from an unrelated working directory and again after ZIP extraction into a path containing spaces with `-OfflineCheck -NoBrowser`, checks the explicit offline response header and export refusal, then requests graceful shutdown. See BUILD-INFO.json and FILE-SHA256SUMS.txt for this package's exact source/build revision and per-file hashes.
